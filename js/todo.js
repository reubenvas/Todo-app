...
document.body.addEventListener('click', (e) => {
  ...
  if (clickedEl.classList.contains('remove-btn')) {
    ...
    removeFromLocalStorage(clickedEl.parentElement.parentElement.id);
  }
  toggleEmptyTag(card);
});
...
function createNote(task, description = '', date, key) {
  const id = key || addToLocalStorage({ task, description, date });
  ...
  const dueDate = createEl('small', `Due in ${calculateDaysLeft(date)} days`, 'due-date');
